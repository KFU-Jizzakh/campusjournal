<x-app-layout>
    ...
    <form method="POST" action="{{ route('submissions.update', $article) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="abstract_ru" name="abstract_ru" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary text-sm" required>{{ old('abstract_ru', $article->abstract_ru) }}</textarea>
                    <x-input-error :messages="$errors->get('abstract_ru')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="abstract_en" :value="__('article.abstract_en_label')" />
                    <textarea id="abstract_en" name="abstract_en" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary text-sm">{{ old('abstract_en', $article->abstract_en) }}</textarea>
                    <x-input-error :messages="$errors->get('abstract_en')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="keywords" :value="__('article.keywords_label')" />
                    <x-text-input id="keywords" name="keywords" type="text" class="mt-1 block w-full" :value="old('keywords', $article->keywords ? implode(', ', $article->keywords) : '')" placeholder="через запятую: образование, методика, РКИ" />
                ...
                <div>
                    ...
                    <textarea id="references" name="references" rows="6" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary text-sm" placeholder="Одна ссылка на строку. DOI будет извлечён автоматически.">{{ old('references', $article->references_text) }}</textarea>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">{{ __('article.new_file_hint') }}</p>
        ...
        @php
            $existingCoauthors = old('coauthors', $article->authors->skip(1)->map(fn ($a) => [
                'full_name' => $a->full_name, 'degree' => $a->degree, 'position' => $a->position,
                'organization' => $a->organization, 'orcid' => $a->orcid,
            ])->values()->toArray());
            $primaryAuthor = $article->authors->first();
        @endphp

        <div class="bg-white rounded-lg border border-gray-200 p-6" x-data="{
            coauthors: @json($existingCoauthors),
            ...
            <div class="border border-gray-100 rounded-lg p-4 mb-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <x-text-input id="author_name" name="author_name" type="text" class="mt-1 block w-full" :value="old('author_name', $primaryAuthor?->full_name ?? auth()->user()->full_name)" required />
                        <x-input-error :messages="$errors->get('author_name')" class="mt-1" />
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <x-input-label for="author_degree" :value="__('article.degree')" />
                            <x-text-input id="author_degree" name="author_degree" type="text" class="mt-1 block w-full" :value="old('author_degree', $primaryAuthor?->degree)" />
                        </div>
                        <div>
                            <x-input-label for="author_position" :value="__('article.position')" />
                            <x-text-input id="author_position" name="author_position" type="text" class="mt-1 block w-full" :value="old('author_position', $primaryAuthor?->position)" />
                        </div>
                    </div>
                    <div>
                        <x-input-label for="author_organization" :value="__('article.affiliation')" />
                        <x-text-input id="author_organization" name="author_organization" type="text" class="mt-1 block w-full" :value="old('author_organization', $primaryAuthor?->organization)" />
                    </div>
                    <div>
                        <x-input-label for="author_orcid" :value="__('article.orcid')" />
                        <x-text-input id="author_orcid" name="author_orcid" type="text" class="mt-1 block w-full" :value="old('author_orcid', $primaryAuthor?->orcid)" placeholder="0000-0000-0000-0000" />
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('submissions.show', $article) }}" class="text-sm text-gray-400 hover:text-gray-600">{{ __('common.cancel') }}</a>
            <x-primary-button>{{ __('article.save_changes') }}</x-primary-button>
        </div>
    </form>
</x-app-layout>
