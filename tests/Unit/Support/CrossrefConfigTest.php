...
test('invalidVariable names the primary variable when its value is invalid', function () {
    expect(CrossrefConfig::invalidVariable('https://journal.example/crossmark-policy'))
        ->toBe('CROSSMARK_POLICY_DOI');
    expect(CrossrefConfig::invalidVariable('https://journal.example/crossmark-policy', '10.5555/legacy-policy'))
        ->toBe('CROSSMARK_POLICY_DOI');
});

test('invalidVariable names the legacy variable when it is the only invalid value', function () {
    expect(CrossrefConfig::invalidVariable(null, 'https://journal.example/crossmark-policy'))
        ->toBe('CROSSMARK_POLICY_URL');
});

test('invalidVariable is null for valid, empty, and unset input', function () {
    expect(CrossrefConfig::invalidVariable('10.5555/crossmark-policy'))->toBeNull();
    expect(CrossrefConfig::invalidVariable(null))->toBeNull();
    expect(CrossrefConfig::invalidVariable(null, '10.5555/legacy-policy'))->toBeNull();
    expect(CrossrefConfig::invalidVariable('', ''))->toBeNull();
});
